...
# pj_name='라그나로크택틱스'
# id_info_df=pd.read_csv('./api/'+pj_name+'_id_info.csv')
# h1=pd.read_csv('./api/'+pj_name+'_history.csv')
# h1=pd.read_pickle('./api/all_history.pkl')
...
h1.loc[h1['활동']=='캠페인 지출 한도 업데이트됨',:] #확인
...
h1.loc[h1['활동']=='광고 세트 예산 업데이트됨','활동 상세 정보'].unique() #확인용
h1.loc[673,'활동 상세 정보'] #확인용
...
text=result_df.iloc[4,:]['활동']
text
mon=[]
if (text=='캠페인 예산이 업데이트됨' or text=='광고 세트 예산 업데이트됨'):
    print('true')

for a in list(text.split('변경 후')):
    mon.append(''.join(re.findall('[0-9]',a)))
    print(mon)
